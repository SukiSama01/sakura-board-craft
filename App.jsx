...
import { 
  ...
  Plus, Layers, Maximize, AlertCircle, Info, X
} from 'lucide-react';
...
export default function KeyboardDesignerApp() {
  ...
  const dragInfo = useRef({ startX: 0, startY: 0, origX: 0, origY: 0, origW: 0, origH: 0, childrenMap: {} });
  ...
  const validateAndSave = () => {
    ...
    if (!allCovered && bgs.length > 0) {
      alert("⚠️ 提示：您的刀图（按键）似乎没有被背景完全覆盖，可能会导致边缘漏白，请检查！");
    }
    ...
    <div className="flex flex-col h-screen bg-pink-50 font-sans text-pink-900 overflow-hidden selection:bg-pink-300 selection:text-white"
      ...
      <header className="flex items-center justify-between px-6 py-3 bg-white border-b border-pink-200 shadow-sm z-10 shrink-0">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={() => setConfirmAction({ isOpen: true, message: '确定要清空当前画布的所有内容吗？', action: () => pushHistory([]) })} className="p-2 text-pink-400 hover:text-pink-600 hover:bg-pink-50 rounded-lg transition" title="清空画布">
            <Trash2 size={18} />
          </button>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-pink-50 p-3 rounded-xl border border-pink-100 space-y-3">
                ...
                <div className="flex space-x-2">
                  <button onClick={mockAIParse} disabled={loadingAI || !baseImage} className="flex-1 py-2 bg-pink-500 text-white rounded-lg text-sm font-medium hover:bg-pink-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center shadow-sm transition">
                    {loadingAI ? 'AI解析中...' : '✨ AI 生成刀图'}
                  </button>
                  <button onClick={() => setShowBaseImage(!showBaseImage)} className="p-2 bg-white border border-pink-200 rounded-lg text-pink-500 hover:bg-pink-50 transition" title="显隐底图">
              ...
              <div className="bg-white p-3 rounded-xl border border-pink-100 space-y-2 shadow-sm">
                <label className="flex items-center text-xs text-pink-700 cursor-pointer hover:text-pink-900">
                  <input type="checkbox" checked={snapEnabled} onChange={e => setSnapEnabled(e.target.checked)} className="mr-2 accent-pink-500" />
                  拖拽背景时智能吸附边缘
                </label>
                <label className="flex items-center text-xs text-pink-700 cursor-pointer hover:text-pink-900">
                  <input type="checkbox" checked={scaleSync} onChange={e => setScaleSync(e.target.checked)} className="mr-2 accent-pink-500" />
                  背景与内部按键联动 (拖拽与缩放)
                </label>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => handleAddElement('background')} className="py-2 flex flex-col items-center bg-white border border-pink-200 rounded-xl hover:bg-pink-50 hover:border-pink-300 transition text-sm text-pink-700">
                  <Square size={20} className="mb-1 text-pink-400" /> 加背景
                </button>
                <button onClick={() => setShowKeyLibraryModal(true)} className="py-2 flex flex-col items-center bg-white border border-pink-200 rounded-xl hover:bg-pink-50 hover:border-pink-300 transition text-sm text-pink-700">
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-white bg-pink-400 px-2 py-1 rounded-md uppercase tracking-wider flex items-center">
                    <Settings size={14} className="mr-1" /> 属性: {selectedElement.type === 'key' ? '按键' : '背景'}
                  </h3>
                  <button onClick={() => { pushHistory(elements.filter(e => e.id !== selectedId)); setSelectedId(null); }} className="text-red-400 hover:text-red-600">
                    <Trash2 size={16} />
                  </button>
                </div>
                ...
                <div className="bg-white border border-pink-200 rounded-xl p-3 space-y-3 shadow-sm">
                  {/* 名称 */}
                  <div>
                    <label className="text-xs text-pink-500 mb-1 block">名称</label>
                    <input type="text" value={selectedElement.name || ''} onChange={e => updateElement(selectedId, { name: e.target.value })} className="w-full text-sm p-1.5 border border-pink-200 rounded outline-none focus:border-pink-400 focus:ring-1 focus:ring-pink-400 bg-pink-50/30" />
                  </div>

                  {/* 几何属性 */}
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-pink-500 mb-1 block">X 坐标</label>
                      <input type="number" value={Math.round(selectedElement.x)} onChange={e => updateElement(selectedId, { x: Number(e.target.value) })} className="w-full text-sm p-1 border border-pink-200 rounded outline-none" />
                    </div>
                    <div>
                      <label className="text-xs text-pink-500 mb-1 block">Y 坐标</label>
                      <input type="number" value={Math.round(selectedElement.y)} onChange={e => updateElement(selectedId, { y: Number(e.target.value) })} className="w-full text-sm p-1 border border-pink-200 rounded outline-none" />
                    </div>
                    <div>
                      <label className="text-xs text-pink-500 mb-1 block">宽度</label>
                      <input type="number" value={Math.round(selectedElement.width)} onChange={e => updateElement(selectedId, { width: Number(e.target.value) })} className="w-full text-sm p-1 border border-pink-200 rounded outline-none" />
                    </div>
                    <div>
                      <label className="text-xs text-pink-500 mb-1 block">高度</label>
                      <input type="number" value={Math.round(selectedElement.height)} onChange={e => updateElement(selectedId, { height: Number(e.target.value) })} className="w-full text-sm p-1 border border-pink-200 rounded outline-none" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-pink-100">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <button onClick={() => updateElement(selectedId, { fillColor: 'linear-gradient(135deg, #fbcfe8, #f472b6)' })} className="px-1 py-1 text-[10px] bg-pink-100 text-pink-600 rounded hover:bg-pink-200" title="应用预设渐变">渐变</button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-1">
                        <input type="color" value={selectedElement.borderColor.startsWith('#') ? selectedElement.borderColor : '#000000'} onChange={e => updateElement(selectedId, { borderColor: e.target.value })} className="w-6 h-6 p-0 border-0 rounded cursor-pointer" />
                        <input type="number" value={selectedElement.borderWidth} onChange={e => updateElement(selectedId, { borderWidth: Number(e.target.value) })} className="w-12 text-xs p-1 border border-pink-200 rounded outline-none" title="边框宽度" />
                      </div>
                    </div>
                    ...
                    <div className="pt-2">
                      <label className="text-xs text-pink-600 mb-1 block">
                        背景图片 <span className="text-[10px] text-pink-400 font-normal ml-1">(可与背景颜色叠加)</span>
                      </label>
                      {selectedElement.backgroundImage ? (
                        <div className="space-y-2">
                          <img src={selectedElement.backgroundImage} alt="bg" className="h-16 w-full object-cover rounded border border-pink-200" />
                          <div className="flex space-x-2">
                            <button onClick={() => updateElement(selectedId, { backgroundImage: null })} className="flex-1 py-1 text-xs text-red-500 bg-red-50 rounded hover:bg-red-100">移除图片</button>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <select value={selectedElement.backgroundSize} onChange={e => updateElement(selectedId, { backgroundSize: e.target.value })} className="p-1 border border-pink-200 rounded text-pink-800 outline-none">
                            ...
                            <select value={selectedElement.backgroundPosition} onChange={e => updateElement(selectedId, { backgroundPosition: e.target.value })} className="p-1 border border-pink-200 rounded text-pink-800 outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-pink-100 bg-pink-50/50 flex justify-between items-center">
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg font-bold text-pink-800">按键库选板</h2>
                </div>
                <div className="h-4 w-px bg-pink-200"></div>
                <button onClick={addLibraryGroup} className="text-sm text-pink-500 hover:text-pink-700 font-medium flex items-center transition">
                ...
                <button onClick={exportKeyLibrary} className="text-sm text-pink-500 hover:text-pink-700 font-medium flex items-center transition">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar bg-gray-50/30">
              {keyLibrary.map((group, gIdx) => (
                <div key={gIdx} className="bg-white p-4 rounded-xl border border-pink-100 shadow-sm">
                  <div className="flex justify-between items-center mb-4 pb-2 border-b border-pink-50">
                    ...
                    <div className="flex space-x-2">
                      <button onClick={() => addLibraryItem(gIdx)} className="text-xs text-pink-500 hover:bg-pink-50 border border-pink-200 px-2 py-1 rounded transition"> + 新增按键 </button>
                      <button onClick={() => setConfirmAction({ isOpen: true, message: '确定要删除整个分组吗？此操作不可恢复。', action: () => removeLibraryGroup(gIdx) })} className="text-xs text-red-400 hover:bg-red-50 border border-red-100 px-2 py-1 rounded transition" title="删除整个分组"> 删除分组 </button>
                    </div>
                  </div>
                    ...
                    {group.items.map((preset, iIdx) => (
                      ...
                      >
                        ...
                        <div 
                          ...
                          onClick={() => { handleAddElement('key', preset); setShowKeyLibraryModal(false); }}
                          title="点击插入到画布"
                        >
      ...
      {confirmAction.isOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-in zoom-in-95 duration-200 border border-pink-100">
            ...
            <p className="text-sm text-gray-600 mb-6 ml-7">{confirmAction.message}</p>
